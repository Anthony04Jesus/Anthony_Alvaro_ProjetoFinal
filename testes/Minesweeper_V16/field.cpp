...
void Field::loadTiles()
{
    ...
    for(int L = 0; L < SIZE_LINE; L++)
    {
        for(int C = 0; C < SIZE_COLUM; C++)
        {
            ...
            connect(&(_tile[i]), SIGNAL(rightClick(bool)), this, SLOT(contador_Right()));
            connect(&(_tile[i]), SIGNAL(leftClick(int,int)), this, SLOT(openTile(int,int)));
            connect(&(_tile[i]), SIGNAL(leftClick(int,int)), this, SLOT(contador_Left()));
...
{
    ResultWindow *dialog = new ResultWindow(result, clique_direito, clique_esquerdo);
    dialog->exec();
    delete dialog;
...
void Field::contador_Right()
{
    clique_direito++;
}

void Field::contador_Left()
{
    clique_esquerdo++;
}
